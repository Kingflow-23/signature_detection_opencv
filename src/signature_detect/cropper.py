...
class Cropper:
    ...
    def __init__(self, min_region_size=5000, border_ratio=0.02):
    ...
    def merge_boxes(self, box_a, box_b) -> list:
        ...
        min_x = min(x_a, x_b)
        min_y = min(y_a, y_b)
        max_x = max(x_a + w_a, x_b + w_b)
        max_y = max(y_a + h_a, y_b + h_b)
    ...
    def _remove_borders(self, box) -> Any:
        ...
        new_w = max(0, w - 2 * border)
        new_h = max(0, h - 2 * border)
        return [x + border, y + border, new_w, new_h]

    def boxes2regions(self, sorted_boxes) -> dict:
        """
        Merge intersecting boxes robustly:
        - Merge until convergence (no overlaps remain).
        - Apply border removal once at the end.
        - Return regions as {idx: [x,y,w,h]}.
        """
        # Work on a list copy
        regions = (
            [list(map(int, box)) for box in sorted_boxes.tolist()]
            if hasattr(sorted_boxes, "tolist")
            else [list(map(int, b)) for b in sorted_boxes]
        )
        ...
        while changed:
                ...
                while merged_any:
                    ...
                    for j in range(i + 1, len(regions)):
                        if taken[j]:
                            continue
                        other = regions[j]
                        if self.is_intersected(cur, other):
                            # merge cur with other
                            cur = self.merge_boxes(cur, other)
                            taken[j] = True
                            merged_any = True
                            changed = True
                taken[i] = True
            ...
            regions = new_regions  # continue until no change

        # Apply borders ONCE, after all merges
        final_regions = {}
        for idx, box in enumerate(regions):
            x, y, w, h = self._remove_borders(box)
            # Guard against negative/zero sizes after border trimming
            if w > 0 and h > 0:
                final_regions[idx] = [x, y, w, h]
